...
def init_db():
    ...
    cursor.execute("DROP TABLE IF EXISTS db_feedback")
    cursor.execute("DROP TABLE IF EXISTS db_campaign")
    ...
    # Table 5: db_feedback (NEW: Customer Survey & Exit Feedback)
    cursor.execute("""
    ...
    # Table 6: db_campaign (NEW: Retention Campaigns)
    cursor.execute("""
    ...
    cursor.executemany("INSERT INTO db_support (customerid, complaint_date, Escalations, csat_score, col_1, comment) VALUES (?,?,?,?,?,?)", supports)

    # Initial Seed Outreach Logs
    outreach_records = [
        (101, "2024-09-16 14:30:00", "Exit Survey & 20% Retention Offer", "aarav.sharma@example.com", 
         "StreamFlix Care: We noticed you left - Help us improve, Aarav!", 
         "Dear Aarav, We noticed your StreamFlix Basic subscription was cancelled. Was it due to the recent price revision in Karnataka? Reply to receive 30% off your next 3 months!", 
         "Subscription got too expensive after recent price revision in Karnataka.", "Price Sensitivity", "Sent", "MSG-101-998", None),
        (102, "2024-09-19 11:15:00", "Exit Survey Email", "priya.patel@example.com", 
         "StreamFlix Exit Survey & Special Discount Offer", 
         "Hi Priya, could you tell us why you decided to leave StreamFlix? We would love to offer you 3 months at 50% off.", 
         "Switched to competitor due to annual combo deal.", "Competitor Switch", "Sent", "MSG-102-999", None),
        (103, "2024-09-06 09:45:00", "Technical Support Check-in Email", "rahul.verma@example.com", 
         "We apologize for the playback lag issue", 
         "Hi Rahul, our engineering team resolved the streaming buffer issue on your device. We credited 1 month free access.", 
         "Video buffering during live matches forced me to cancel.", "Technical Glitch", "Sent", "MSG-103-1000", None)
    ]
    cursor.executemany("""
    INSERT INTO db_outreach_log (customerid, contact_date, contact_type, email_address, email_subject, email_body, customer_feedback, categorized_reason, status, message_id, error_message)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)
    """, outreach_records)

    # Initial Feedback Records
    feedbacks = [
        (101, "Price Sensitivity", "Subscription became too expensive after recent price revision.", "2024-09-16 15:00:00", "Offer 20% discount on Annual Tier"),
        (102, "Competitor Switch", "Switched to competitor for annual combo deal.", "2024-09-19 12:00:00", "Match competitor annual combo deal"),
        (103, "Technical Glitch", "App buffering during live cricket matches.", "2024-09-06 10:30:00", "Priority tech SLA + 1 Month Free Access")
    ]
    cursor.executemany("INSERT INTO db_feedback (customerid, reason_category, comment, submitted_at, recommended_action) VALUES (?,?,?,?,?)", feedbacks)

    # Initial Campaign Records
    campaigns = [
        (1, "Price Sensitivity Retention", "High Risk + Basic Monthly", 6, 6, 3, "2024-09-20 10:00:00"),
        (2, "Karnataka Regional Recovery", "Karnataka Churned Subscribers", 5, 5, 2, "2024-09-21 09:00:00")
    ]
    cursor.executemany("INSERT INTO db_campaign VALUES (?,?,?,?,?,?,?)", campaigns)

    conn.commit()
    conn.close()
    print("Database customer_churn.db initialized successfully with 6 tables!")
